	...
	switch(m_type)
	{
		case LedType::LedTemp:
			...
			if (!colorTemp.empty())
			{
				try
				{
					int cTemp = std::stoi(colorTemp) ;
					color.mode = ColorModeTemp;
					color.t = cTemp & 0xff;
				}
				catch(std::exception& e)
				{
					std::cout<<"error: get color temp("<<colorTemp<<") failed "<<e.what()<<std::endl;
					return false;
				}
			}
		...
		case LedType::LedRGB:
			...
			if (!ledRGB.empty())
			{
				...
				{
					std::cout<<"error: get color rgb("<<ledRGB<<") failed "<<e.what()<<std::endl;
					return false;
				}
			}
		break;
		case LedType::LedRGBTemp:
			...
			{
				std::cout<<"error: get color rgb("<<ledRGB<<")  temp("<<colorTemp<<") failed "<<std::endl;
				return false;
			}
